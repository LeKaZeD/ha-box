...
void Model::setClock(uint8_t hh, uint8_t mm, uint8_t ss, uint32_t receivedAtMs)
{
  if (hh > 23 || mm > 59 || ss > 59) {
    Serial2.printf("[Model] Invalid clock: %u:%u:%u\n", hh, mm, ss);
    return;
  }
  home.hour = hh;
  home.minute = mm;
...
void Model::updateClockDisplay(uint32_t nowMs)
{
  ...
  // Cap to 2 minutes: if the clock handler is late or stopped, show stale time
  // rather than advancing by an arbitrary amount. Unsigned subtraction handles
  // the millis() 49-day rollover correctly, but a very large elapsed value
  // (e.g. handler paused for hours) would produce wrong display time.
  if (elapsedSec > 120u) elapsedSec = 0u;
...
void Model::setWarnings(uint8_t count)
{
  if (home.warnings_count == count) return;
  home.warnings_count = count;
  dirty |= DK_HM_Alerts;
}

void Model::setErrors(uint8_t count)
{
  if (home.errors_count == count) return;
  home.errors_count = count;
  dirty |= DK_HM_Alerts;
}
