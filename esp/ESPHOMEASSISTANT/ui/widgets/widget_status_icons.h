...
struct StatusIconsWidget : public WidgetBase<StatusIconsWidget>
{
  ...
  static constexpr int16_t IR_X = 128;
  static constexpr int16_t MHZ433_X = 160;
  static constexpr int16_t MATTER_X = 192;
  ...
  {
    if (dirtyMask & (DK_HM_Wifi | DK_HM_BLE | DK_HM_Zigbee | DK_HM_Thread | DK_HM_Ir | DK_HM_Mhz433 | DK_HM_Matter)) dirty = true;
  }
  ...
  void draw(DisplayT& display, const Model& model)
  {
    ...
    const uint8_t* wifiBmp = model.home.wifi_ok ? WIFI_DATA_24x24 : WIFI_STOP_DATA_24x24;
    const uint8_t* bleBmp  = model.home.ble_ok  ? BLE_DATA_24x24  : BLE_STOP_DATA_24x24;
    ...
    const uint8_t* mhz433Bmp = model.home.mhz433_ok ? Z_WAVE_DATA_24x24 : WHITE_24x24;
    const uint8_t* matterBmp = model.home.matter_ok ? MATTER_DATA_24x24 : WHITE_24x24;

    drawIcon_0Black(display, xw, yw, wifiBmp, ICON_W, ICON_H, GxEPD_BLACK, GxEPD_WHITE, tmp);
    drawIcon_0Black(display, xb, yb, bleBmp,  ICON_W, ICON_H, GxEPD_BLACK, GxEPD_WHITE, tmp);
