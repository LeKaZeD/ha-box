...
void AsciiProto::processLine(char* line) {
  ...
  // Toute ligne valide reçue met à jour lastSeen
  // On ne le fait qu'après parsing réussi pour éviter le bruit
  Ack ack;
  ...
  if (parseMessage(line, msg)) {
    ...
    // Auto-ACK avant action (et dédup gérée ensuite)
    autoAckForMessage(msg);

    // Déduplication: si duplicate, on ne ré-exécute pas
...
void AsciiProto::autoAckForMessage(const Message& msg) {
  // ACK immédiat: autorisation rapide (allowlist) si callback présent
  if (m_authFn) {
